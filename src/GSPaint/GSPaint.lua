--[[
    ...
    Authors: grofix_splash
    GitHub: https://github.com/H1lkaaaGD/gspaint
    Telegramm: https://t.me/gspaint
--]]
...
local ClosureBindings = {
    function()local wax,script,require=ImportGlobals(1)local ImportGlobals return (function(...)if getgenv().gspaint_loading then print("[gspaint] Loading stopped. (ERROR: Loading)"); return end
if getgenv().gspaint_loaded then print("[gspaint] Loading stopped. (ERROR: Already loaded)"); return end

getgenv().gspaint_loading = true
...
--// Functions \\--
Script._gspaint_custom_captions = Instance.new("ScreenGui") do
    local Frame = Instance.new("Frame", Script._gspaint_custom_captions)
    local TextLabel = Instance.new("TextLabel", Frame)
    local UITextSizeConstraint = Instance.new("UITextSizeConstraint", TextLabel)

    Script._gspaint_custom_captions.Parent = shared.ReplicatedStorage
    Script._gspaint_custom_captions.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    ...
    function Script.Functions.Captions(caption: string)
        ...
        if Script._gspaint_custom_captions.Parent == shared.ReplicatedStorage then
            local success = pcall(function()
                Script._gspaint_custom_captions.Parent = if gethui then gethui() else shared.CoreGui
            end)

            if not success then
                Script._gspaint_custom_captions.Parent = shared.PlayerGui
            end 
        end
...
shared.Library:OnUnload(function()
    ...
    if Script.IsMines then
        local acbypasspart = workspace:FindFirstChild("_internal_gspaint_acbypassprogress")
        if acbypasspart then acbypasspart:Destroy() end
    end

    if Script.IsRooms then
        if workspace:FindFirstChild("_internal_gspaint_pathfinding_nodes") then
            workspace:FindFirstChild("_internal_gspaint_pathfinding_nodes"):Destroy()
        end
        if workspace:FindFirstChild("_internal_gspaint_pathfinding_block") then
            workspace:FindFirstChild("_internal_gspaint_pathfinding_block"):Destroy()
        end
    end

    if Script._gspaint_custom_captions then
        Script._gspaint_custom_captions:Destroy()
    end
    ...
    Toggles.TheMinesAnticheatBypass:OnChanged(function(value)
        if value then
            local progressPart = Instance.new("Part", shared.Workspace) do
                ...
                progressPart.Name = "_internal_gspaint_acbypassprogress"
            ...
            if workspace:FindFirstChild("_internal_gspaint_acbypassprogress") then workspace:FindFirstChild("_internal_gspaint_acbypassprogress"):Destroy() end
...
elseif Script.IsRooms then
    ...
    local _internal_gspaint_pathfinding_nodes = Instance.new("Folder", shared.Workspace) do
        _internal_gspaint_pathfinding_nodes.Name = "_internal_gspaint_pathfinding_nodes"
    end

    local _internal_gspaint_pathfinding_block = Instance.new("Folder", shared.Workspace) do
        _internal_gspaint_pathfinding_block.Name = "_internal_gspaint_pathfinding_block"
    end

    Toggles.ShowAutoRoomsPathNodes:OnChanged(function(value)
        for _, node in pairs(_internal_gspaint_pathfinding_nodes:GetChildren()) do
            node.Transparency = value and 0.5 or 1
        end
        for _, nodeBlock in pairs(_internal_gspaint_pathfinding_block:GetChildren()) do
    ...
    Toggles.AutoRooms:OnChanged(function(value)
        ...
        local function nodeCleanup()
            _internal_gspaint_pathfinding_nodes:ClearAllChildren()
            _internal_gspaint_pathfinding_block:ClearAllChildren()
        ...
        if value then
            ...
            local function createNewBlockedPoint(point: PathWaypoint)
                local block = Instance.new("Part", _internal_gspaint_pathfinding_block)
                local pathMod = Instance.new("PathfindingModifier", block)
                pathMod.Label = "_ms_pathBlock"

                block.Name = "_gspaint_blocked_path"
            ...
            local function doAutoRooms()
                ...
                if path.Status == Enum.PathStatus.Success then
                    ...
                    for i, waypoint in pairs(waypoints) do
                        local node = Instance.new("Part", _internal_gspaint_pathfinding_nodes) do
                        ...
                        if not moveToFinished or not Toggles.AutoRooms.Value then
                            ...
                            if isEntitySpawned and not shared.RootPart.Anchored and pathfindingGoal.Parent.Name ~= "Rooms_Locker" then
                                ...
                                if not Toggles.AutoRooms.Value then
                                    ...
                                    if _internal_gspaint_pathfinding_nodes:FindFirstChild("_internal_node_" .. i) then
                                        _internal_gspaint_pathfinding_nodes:FindFirstChild("_internal_node_" .. i):Destroy()
                                    end
                                end
    ...
    shared.Connect:GiveSignal(clientRemote.Haste.Remote.OnClientEvent:Connect(function(value)
        if not value and Toggles.NotifyEntity.Value then
            haste_incoming_progress = Instance.new("Part", shared.Workspace); do
                ...
                haste_incoming_progress.Name = "_internal_gspaint_haste"
...
local function HasAlreadyPathfind(nodesFolder: Folder): boolean
    local hasPathfind = nodesFolder:GetAttribute("_gspaint_nodes_pathfind")
    return hasPathfind
end
...
local function PathfindGetNodes(room: Model): tSortedNodes | nil
    ...
    for _, node: Part in ipairs(nodeArray:GetChildren()) do
        ...
        local realNumber = node:GetAttribute("_gspaint_real_node")
        if realNumber then table.insert(Nodes.real, node) continue end
        --check for fake nodes
        local fakeNumber = node:GetAttribute("_gspaint_fake_node")
...
local function HasAlreadyDestroyed(room: Model): boolean
    ...
    local result = nodesFolder:GetAttribute("_gspaint_player_teleported") ~= nil
...
--[[
    Use "_gspaint_player_teleported" to track the status of the Teleport meaning that:
    _gspaint_player_teleported = nil   ==> Node not destroyed
    _gspaint_player_teleported = false ==> Node destroyed
    _gspaint_player_teleported = true ==> Nodes was destroyed + Player sucessfully teleported.
]]
...
function Script.Functions.Minecart.Teleport(room: Model)
    ...
    if not HasAlreadyDestroyed(room) then
        ...
        room:SetAttribute("_gspaint_player_teleported", false)
    end

    local hasAlreadyTeleported = room:GetAttribute("_gspaint_player_teleported")
    ...
    if roomNumber == 45 and not hasAlreadyTeleported then
        task.spawn(function()
            ...
            if workspace:FindFirstChild("_internal_gspaint_minecart_teleport") then 
                workspace:FindFirstChild("_internal_gspaint_minecart_teleport"):Destroy() 
            end

            local progressPart = Instance.new("Part", workspace) do
                progressPart.Anchored = true
                progressPart.CanCollide = false
                progressPart.Name = "_internal_gspaint_minecart_teleport"
                progressPart.Transparency = 1
            end
            shared.Notify:Alert({
        ...
        if workspace:FindFirstChild("_internal_gspaint_minecart_teleport") then workspace:FindFirstChild("_internal_gspaint_minecart_teleport"):Destroy() end
        shared.Notify:Alert({
            Title = "Minecart Teleport",
        ...
        while startRoomNumber >= 45 and startRoomNumber <= 49 do
            ...
            GetRoom:SetAttribute("_gspaint_player_teleported", true)
...
function Script.Functions.Minecart.Pathfind(room: Model, forcePathfind: boolean)
    ...
    nodesFolder:SetAttribute("_gspaint_nodes_pathfind", true)
    PathfindSetNodes(nodesList.real, "_gspaint_real_node")
    PathfindSetNodes(nodesList.fake, "_gspaint_fake_node")
...
return Fly
    ...
    Debug = true, --or getgenv().gspaint_dev_mode,
...
if getgenv().gspaint_disable_addons or getgenv().gspaint_addons_activated then 
    return {}
end
...
task.spawn(function()
    ...
    getgenv().gspaint_addons_activated = true
    ...
    repeat task.wait() until getgenv().gspaint_loaded == true
